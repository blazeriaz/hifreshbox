...
class RecipeRepository implements \Freshbox\Recipes\Api\RecipeRepositoryInterface
{
   ...
   /* $model->setData('recipe_id', $recipe->getId());
    ...
    public function updatepdf($pdfdata)
    {
	  ...
	  $file_name = time().$pdfdata['file'];
	  $pdf_dir_location = $this->_filesystem->getDirectoryRead(DirectoryList::MEDIA)->getAbsolutePath().'recipe-pdf';
	  $pdf_location = $this->_filesystem->getDirectoryRead(DirectoryList::MEDIA)->getAbsolutePath().'recipe-pdf/'.$file_name;
	   $pdf_binary_content = base64_decode($pdfdata['content']['base64_encoded_data']);
	  file_put_contents($pdf_location, $pdf_binary_content);
	  $product_id = $pdfdata['product_id'];
	  
	  $product = $this->productFactory->create();
	  $product->load($product_id)->setData('pdf_upload', $file_name)->save();
	  $pdfdata['file'] = $file_name;
	  $pdfdata['file_dir'] = $pdf_dir_location;
		return  $pdfdata;
