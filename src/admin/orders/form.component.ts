...
export class OrderFormComponent implements OnInit {
    ...
    loadFormData() {
        ...
        if(!orderId) return;
        this.loadFormRequests.push(
            this.rest.getItem(orderId, 'customers/' + orderId).subscribe(order => {
                this.order = order;
                this.checkAllFormDataLoaded();
            })
        );
    ...
    initEditForm() {
        ...
        if(this.order.id) {            
            this.order.addresses.map((address, i) => {
                if(address.default_shipping) {
                    this.indexDefaultAddress = i;
                }
                this.orderForm.controls['addresses'].push(this.initAddressForm(address));
            });
            if(this.order.addresses.length > 0) {
    ...
    getActiveCountry() {
        let country_id = this.orderForm.value.addresses[this.indexEditAddress].country_id;
        return this.countries.filter(x => x.id == country_id);
    }

    getActiveCountryRegions() {
        let available_regions = this.available_regions[this.orderForm.value.addresses[this.indexEditAddress].country_id];
    ...
    getActiveRegion() {
        ...
        if(!regions) return;        
        let region_id = this.orderForm.value.addresses[this.indexEditAddress].region_id;
    ...
    setAddressClass(i: number) {
        return {            
            ...
            'bg-danger text-white' : this.submitted && this.orderForm.controls.addresses.controls[i].invalid,          
        };
    }

    setGender(gender) {
        this.orderForm.patchValue({gender : gender});
    }

    getGender() {
        return this.orderForm.value.gender;
    }

    patchAddressValue(i: number, values: object) {
        if(this.orderForm.controls['addresses'].controls[i])
            this.orderForm.controls['addresses'].controls[i].patchValue(values);
    }
    ...
    setInputErrorClass(input) {
        let invalid = this.orderForm.get(input).invalid && this.submitted;
    ...
    setAddressInputClass(input) {
        let invalid = this.orderForm.controls['addresses'].controls[this.indexEditAddress].get(input).invalid;
    ...
    deleteAddress(i: number) {
        ...
        if(confirm('Are you sure want to delete the address?')) {
            this.orderForm.controls['addresses'].removeAt(i);
            this.doEditAddress(0);
        }
    }
    ...
    addNewAddress() {
        ...
        this.orderForm.controls['addresses'].push(this.initAddressForm(newAddress));
        this.doEditAddress(this.orderForm.controls['addresses'].controls.length - 1);
    }

    noticeOrderSaved = function() {
    ...
    saveOrder() {
        ...
        if (this.orderForm.valid) {                   
            ...
            this.rest.saveItem(orderId, {customer: this.orderForm.value}, "customers/" + orderId).subscribe(data => {
                this.noticeOrderSaved();                  
            });
        } else {
